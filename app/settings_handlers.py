...
async def set_params(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    bot_params = user_context.bot_params
    ...
    try:
        ...
        old_params = {
            "profit_percentage": bot_params.profit_percentage,
            "fall_percentage": bot_params.fall_percentage,
            "delay_seconds": bot_params.delay_seconds,
            "order_size": bot_params.order_size,
        }
        
        bot_params.set_params(profit_percentage, fall_percentage, delay_seconds, order_size)
        user_context.save_user_params()
        
        if (
            old_params["profit_percentage"] == bot_params.profit_percentage and
            old_params["fall_percentage"] == bot_params.fall_percentage and
            old_params["delay_seconds"] == bot_params.delay_seconds and
            old_params["order_size"] == bot_params.order_size
        ):
            await update.message.reply_text(
                f'Параметры не изменились.\n'
                f'Текущие параметры:\n'
                f'Процент прибыли: {bot_params.profit_percentage}%\n'
                f'Процент падения: {bot_params.fall_percentage}%\n'
                f'Задержка: {bot_params.delay_seconds} секунд\n'
                f'Размер ордера: ${bot_params.order_size}'
            )
        else:
            await update.message.reply_text(
